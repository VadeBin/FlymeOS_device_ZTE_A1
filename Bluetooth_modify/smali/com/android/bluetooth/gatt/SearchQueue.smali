.class Lcom/android/bluetooth/gatt/SearchQueue;
.super Ljava/lang/Object;
.source "SearchQueue.java"


# annotations
.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        Lcom/android/bluetooth/gatt/SearchQueue$Entry;
    }
.end annotation


# static fields
.field private static final TAG:Ljava/lang/String; = "SearchQueue"


# instance fields
.field private mEntries:Ljava/util/List;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/util/List",
            "<",
            "Lcom/android/bluetooth/gatt/SearchQueue$Entry;",
            ">;"
        }
    .end annotation
.end field


# direct methods
.method constructor <init>()V
    .locals 1

    .prologue
    .line 29
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    .line 53
    new-instance v0, Ljava/util/ArrayList;

    invoke-direct {v0}, Ljava/util/ArrayList;-><init>()V

    iput-object v0, p0, Lcom/android/bluetooth/gatt/SearchQueue;->mEntries:Ljava/util/List;

    return-void
.end method


# virtual methods
.method add(IIIJJ)V
    .locals 4
    .param p1, "connId"    # I
    .param p2, "srvcType"    # I
    .param p3, "srvcInstId"    # I
    .param p4, "srvcUuidLsb"    # J
    .param p6, "srvcUuidMsb"    # J

    .prologue
    .line 57
    new-instance v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;

    invoke-direct {v0, p0}, Lcom/android/bluetooth/gatt/SearchQueue$Entry;-><init>(Lcom/android/bluetooth/gatt/SearchQueue;)V

    .line 58
    .local v0, "entry":Lcom/android/bluetooth/gatt/SearchQueue$Entry;
    iput p1, v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;->connId:I

    .line 59
    iput p2, v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;->srvcType:I

    .line 60
    iput p3, v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;->srvcInstId:I

    .line 61
    iput-wide p4, v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;->srvcUuidLsb:J

    .line 62
    iput-wide p6, v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;->srvcUuidMsb:J

    .line 63
    const-wide/16 v2, 0x0

    iput-wide v2, v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;->charUuidLsb:J

    .line 64
    iget-object v1, p0, Lcom/android/bluetooth/gatt/SearchQueue;->mEntries:Ljava/util/List;

    invoke-interface {v1, v0}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    .line 65
    return-void
.end method

.method add(IIIJJIJJ)V
    .locals 2
    .param p1, "connId"    # I
    .param p2, "srvcType"    # I
    .param p3, "srvcInstId"    # I
    .param p4, "srvcUuidLsb"    # J
    .param p6, "srvcUuidMsb"    # J
    .param p8, "charInstId"    # I
    .param p9, "charUuidLsb"    # J
    .param p11, "charUuidMsb"    # J

    .prologue
    .line 71
    new-instance v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;

    invoke-direct {v0, p0}, Lcom/android/bluetooth/gatt/SearchQueue$Entry;-><init>(Lcom/android/bluetooth/gatt/SearchQueue;)V

    .line 72
    .local v0, "entry":Lcom/android/bluetooth/gatt/SearchQueue$Entry;
    iput p1, v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;->connId:I

    .line 73
    iput p2, v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;->srvcType:I

    .line 74
    iput p3, v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;->srvcInstId:I

    .line 75
    iput-wide p4, v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;->srvcUuidLsb:J

    .line 76
    iput-wide p6, v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;->srvcUuidMsb:J

    .line 77
    iput p8, v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;->charInstId:I

    .line 78
    iput-wide p9, v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;->charUuidLsb:J

    .line 79
    iput-wide p11, v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;->charUuidMsb:J

    .line 80
    iget-object v1, p0, Lcom/android/bluetooth/gatt/SearchQueue;->mEntries:Ljava/util/List;

    invoke-interface {v1, v0}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    .line 81
    return-void
.end method

.method clear()V
    .locals 1

    .prologue
    .line 137
    iget-object v0, p0, Lcom/android/bluetooth/gatt/SearchQueue;->mEntries:Ljava/util/List;

    invoke-interface {v0}, Ljava/util/List;->clear()V

    .line 138
    return-void
.end method

.method isEmpty()Z
    .locals 2

    .prologue
    .line 118
    const-string v0, "SearchQueue"

    iget-object v1, p0, Lcom/android/bluetooth/gatt/SearchQueue;->mEntries:Ljava/util/List;

    invoke-virtual {v1}, Ljava/lang/Object;->toString()Ljava/lang/String;

    move-result-object v1

    invoke-static {v0, v1}, Landroid/util/Log;->i(Ljava/lang/String;Ljava/lang/String;)I

    .line 119
    iget-object v0, p0, Lcom/android/bluetooth/gatt/SearchQueue;->mEntries:Ljava/util/List;

    invoke-interface {v0}, Ljava/util/List;->isEmpty()Z

    move-result v0

    return v0
.end method

.method isEmpty(I)Z
    .locals 6
    .param p1, "connId"    # I

    .prologue
    .line 123
    invoke-virtual {p0}, Lcom/android/bluetooth/gatt/SearchQueue;->isEmpty()Z

    move-result v0

    .line 124
    .local v0, "bEmpty":Z
    if-nez v0, :cond_1

    .line 125
    const/4 v0, 0x1

    .line 126
    iget-object v3, p0, Lcom/android/bluetooth/gatt/SearchQueue;->mEntries:Ljava/util/List;

    invoke-interface {v3}, Ljava/util/List;->iterator()Ljava/util/Iterator;

    move-result-object v2

    .local v2, "i$":Ljava/util/Iterator;
    :cond_0
    :goto_0
    invoke-interface {v2}, Ljava/util/Iterator;->hasNext()Z

    move-result v3

    if-eqz v3, :cond_1

    invoke-interface {v2}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v1

    check-cast v1, Lcom/android/bluetooth/gatt/SearchQueue$Entry;

    .line 127
    .local v1, "entry":Lcom/android/bluetooth/gatt/SearchQueue$Entry;
    iget v3, v1, Lcom/android/bluetooth/gatt/SearchQueue$Entry;->connId:I

    if-ne p1, v3, :cond_0

    .line 128
    const/4 v0, 0x0

    goto :goto_0

    .line 132
    .end local v1    # "entry":Lcom/android/bluetooth/gatt/SearchQueue$Entry;
    .end local v2    # "i$":Ljava/util/Iterator;
    :cond_1
    const-string v3, "SearchQueue"

    new-instance v4, Ljava/lang/StringBuilder;

    invoke-direct {v4}, Ljava/lang/StringBuilder;-><init>()V

    const-string v5, "isEmpty: connId="

    invoke-virtual {v4, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v4

    invoke-virtual {v4, p1}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    move-result-object v4

    const-string v5, ", bEmpty="

    invoke-virtual {v4, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v4

    invoke-virtual {v4, v0}, Ljava/lang/StringBuilder;->append(Z)Ljava/lang/StringBuilder;

    move-result-object v4

    invoke-virtual {v4}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v4

    invoke-static {v3, v4}, Landroid/util/Log;->i(Ljava/lang/String;Ljava/lang/String;)I

    .line 133
    return v0
.end method

.method pop()Lcom/android/bluetooth/gatt/SearchQueue$Entry;
    .locals 3

    .prologue
    const/4 v2, 0x0

    .line 84
    iget-object v1, p0, Lcom/android/bluetooth/gatt/SearchQueue;->mEntries:Ljava/util/List;

    invoke-interface {v1, v2}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;

    .line 85
    .local v0, "entry":Lcom/android/bluetooth/gatt/SearchQueue$Entry;
    iget-object v1, p0, Lcom/android/bluetooth/gatt/SearchQueue;->mEntries:Ljava/util/List;

    invoke-interface {v1, v2}, Ljava/util/List;->remove(I)Ljava/lang/Object;

    .line 86
    return-object v0
.end method

.method popByConnId(I)Lcom/android/bluetooth/gatt/SearchQueue$Entry;
    .locals 5
    .param p1, "connId"    # I

    .prologue
    .line 91
    const/4 v0, 0x0

    .line 93
    .local v0, "entry":Lcom/android/bluetooth/gatt/SearchQueue$Entry;
    const-string v2, "SearchQueue"

    new-instance v3, Ljava/lang/StringBuilder;

    invoke-direct {v3}, Ljava/lang/StringBuilder;-><init>()V

    const-string v4, "popByConnId: connId="

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v3

    invoke-virtual {v3, p1}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    move-result-object v3

    invoke-virtual {v3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v3

    invoke-static {v2, v3}, Landroid/util/Log;->i(Ljava/lang/String;Ljava/lang/String;)I

    .line 94
    const-string v2, "SearchQueue"

    new-instance v3, Ljava/lang/StringBuilder;

    invoke-direct {v3}, Ljava/lang/StringBuilder;-><init>()V

    const-string v4, "popByConnId:mEntries = "

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v3

    iget-object v4, p0, Lcom/android/bluetooth/gatt/SearchQueue;->mEntries:Ljava/util/List;

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/Object;)Ljava/lang/StringBuilder;

    move-result-object v3

    invoke-virtual {v3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v3

    invoke-static {v2, v3}, Landroid/util/Log;->i(Ljava/lang/String;Ljava/lang/String;)I

    .line 95
    iget-object v2, p0, Lcom/android/bluetooth/gatt/SearchQueue;->mEntries:Ljava/util/List;

    invoke-interface {v2}, Ljava/util/List;->iterator()Ljava/util/Iterator;

    move-result-object v1

    .local v1, "it":Ljava/util/Iterator;, "Ljava/util/Iterator<Lcom/android/bluetooth/gatt/SearchQueue$Entry;>;"
    :cond_0
    invoke-interface {v1}, Ljava/util/Iterator;->hasNext()Z

    move-result v2

    if-eqz v2, :cond_1

    .line 96
    invoke-interface {v1}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v0

    .end local v0    # "entry":Lcom/android/bluetooth/gatt/SearchQueue$Entry;
    check-cast v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;

    .line 97
    .restart local v0    # "entry":Lcom/android/bluetooth/gatt/SearchQueue$Entry;
    iget v2, v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;->connId:I

    if-ne v2, p1, :cond_0

    .line 98
    invoke-interface {v1}, Ljava/util/Iterator;->remove()V

    .line 103
    :cond_1
    const-string v2, "SearchQueue"

    new-instance v3, Ljava/lang/StringBuilder;

    invoke-direct {v3}, Ljava/lang/StringBuilder;-><init>()V

    const-string v4, "popByConnId:mEntries = "

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v3

    iget-object v4, p0, Lcom/android/bluetooth/gatt/SearchQueue;->mEntries:Ljava/util/List;

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/Object;)Ljava/lang/StringBuilder;

    move-result-object v3

    const-string v4, ", entry="

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v3

    invoke-virtual {v3, v0}, Ljava/lang/StringBuilder;->append(Ljava/lang/Object;)Ljava/lang/StringBuilder;

    move-result-object v3

    invoke-virtual {v3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v3

    invoke-static {v2, v3}, Landroid/util/Log;->i(Ljava/lang/String;Ljava/lang/String;)I

    .line 104
    return-object v0
.end method

.method removeConnId(I)V
    .locals 3
    .param p1, "connId"    # I

    .prologue
    .line 109
    iget-object v2, p0, Lcom/android/bluetooth/gatt/SearchQueue;->mEntries:Ljava/util/List;

    invoke-interface {v2}, Ljava/util/List;->iterator()Ljava/util/Iterator;

    move-result-object v1

    .local v1, "it":Ljava/util/Iterator;, "Ljava/util/Iterator<Lcom/android/bluetooth/gatt/SearchQueue$Entry;>;"
    :cond_0
    :goto_0
    invoke-interface {v1}, Ljava/util/Iterator;->hasNext()Z

    move-result v2

    if-eqz v2, :cond_1

    .line 110
    invoke-interface {v1}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;

    .line 111
    .local v0, "entry":Lcom/android/bluetooth/gatt/SearchQueue$Entry;
    iget v2, v0, Lcom/android/bluetooth/gatt/SearchQueue$Entry;->connId:I

    if-ne v2, p1, :cond_0

    .line 112
    invoke-interface {v1}, Ljava/util/Iterator;->remove()V

    goto :goto_0

    .line 115
    .end local v0    # "entry":Lcom/android/bluetooth/gatt/SearchQueue$Entry;
    :cond_1
    return-void
.end method
